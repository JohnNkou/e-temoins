...
export default function Card({url,title='', link}){
	...
	useEffect(()=>{
		axios.get(url).then((response)=>{
			...
			console.error("Error white loading "+url);
			console.error(error);
		})
	},[true]);
	...
	return (
		<div onClick={changeLocation} class="col-xl-3 col-xxl-3 col-lg-6 col-sm-6">
				...
				<div class="card-body">
					<div class="media align-items-center">
						...
						<div class="media-body text-end feature-icon-text">
							<p class="fs-18 text-white mb-2">{title}</p>
							<span class="fs-48 text-white font-w600">{total}</span>
						</div>
					</div>
				</div>
			</div>
